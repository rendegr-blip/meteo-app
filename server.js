...
async function getWeatherFromWeatherAPI(lat, lon) {
  ...
  try {
    ...
    console.log('WeatherAPI non disponible:', error.message);
    return null;
  }
}
...
app.get('/api/weather/:city', async (req, res) => {
  try {
    ...
    if (weatherData.length === 0) {
      return res.status(500).json({ error: 'Impossible de recuperer les donnees meteo' });
    }
...
const hasReactBuild = fs.existsSync(path.join(buildPath, 'index.html'));

if (process.env.NODE_ENV === 'production' || hasReactBuild) {
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`Serveur meteo lance sur le port ${PORT}`);
  });
}
